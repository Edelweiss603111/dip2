...
export class DomhandlerService {
    ...
    // добавляет несколько классов элементу
    ...
    // возвращает список всех "родственников"
    ...
    // возвращает все элементы, соответствующие указанному селектору
    ...
    // возвращает номер элемента среди всех элементов родителя
    ...
    // возвращает высоту проскролленного окна
    public getWindowScrollTop(): number {
        let doc = document.documentElement;
        return (window.pageYOffset || doc.scrollTop) - (doc.clientTop || 0);
    }

    // возвращает ширину проскролленного окна
    public getWindowScrollLeft(): number {
        let doc = document.documentElement;
        return (window.pageXOffset || doc.scrollLeft) - (doc.clientLeft || 0);
    }

    // среди всех элементов с данным селектором проверяет, если ли нужный нам элемент
    public matches(element, selector: string): boolean {
        var p = Element.prototype;
        var f = p['matches'] || p.webkitMatchesSelector || p['mozMatchesSelector'] || p.msMatchesSelector || function (s) {
            return [].indexOf.call(document.querySelectorAll(s), this) !== -1;
        };
        return f.call(element, selector); // первый аргумент задает значение this внутри функции. Итоговый вид: document.querySelectorAll(selector).IndexOf(element)
    }

    // возвращает полную ширину с учетом margin
    public getOuterWidth(el, margin?) {
        let width = el.offsetWidth; // offsetWidth содержит полную ширину элемента (включает собственно ширину элемента, ширину границ, padding, scrollbar)

        if (margin) {
            let style = getComputedStyle(el); // получает все стили элемента
    ...
    // возвращает горизонтальный margin
    public getHorizontalMargin(el) {
        let style = getComputedStyle(el);
        return parseFloat(style.marginLeft) + parseFloat(style.marginRight);
    }

    // возвращает ширину вместе с паддингами - а нахрена оно суммирует паддинги, если они есть в offsetWidth?
    ...
    // озвращает ширину без паддингов
    public width(el) {
        let width = el.offsetWidth;
        let style = getComputedStyle(el);
  ...
  // возвращает высоту вместе с паддингами - а нахрена оно суммирует паддинги, если они есть в offsetWidth?
    ...
    // вовращает высоту с margin'ом, если он задан
   ...
   // вовращает высоту без паддинга
    ...
    // координаты элемента относительно верхнего левого угла
    public getOffset(el) {
        let rect = el.getBoundingClientRect(); // возвращает координаты элемента, под которыми понимаются размеры «воображаемого прямоугольника», который охватывает весь элемент
    ...
    clearSelection(): void {
        if(window.getSelection) { // Метод getSelection возвращает объект Selection, представленный в виде диапазона текста, который пользователь выделил на странице.
            if(window.getSelection().empty) { // При добавлении в параметр пустую строку или вспользовавшись методом Selection.toString() вам вернут строку, которую выделил пользователь.
